...
def main(config):
    ...
    with open("../new/back-pos.json", "r") as f:
    ...
    for i in np.arange(0,video.duration,1/15):
        ...
        img = imgs[0]/255
        h, w, _ = img.shape
        xywh = load_dict.get(str(m)).get("T")
        xyxy = []
        pred = []
        if not xywh:
            continue
        ...
        #     f.write("\n片段第"+ str(m) + '/' + str(int(video.duration*15)) + '帧' + ':')
        # f.close()
        #
        ...
        if yolo_preds.get("pred")[0].shape[0]:
                ...
                for list2 in slowfaster_preds:
                    ...
                    for j in range(0, 80):
                        if j == 60 or j == 79:
                            ...
                            str2['Interaction'] = str1[1]
                        else:
                            str1[2][ava_labelnames[j + 1]] = str(round(list2.tolist()[j] * 100, 3)) + '%'
                            str2['Body'] = str1[2]
                    peostr = 'T'
                    actionall[peostr] = str2
                    resultall[str(m)] = actionall

                slowfaster_preds = slowfaster_preds.cpu()
                value, indices = torch.topk(slowfaster_preds, 4)
            for tid,avalabel in zip(pred_tensor[0][:,5].tolist(),indices.tolist()):
                if avalabel[0] == 60 or avalabel[0] == 79:
                    id_to_ava_labels[tid] = "{Head}:"+ava_labelnames[avalabel[0]+1]
                elif avalabel[0] in np.arange(63, 79, 1):
                    id_to_ava_labels[tid] = "{Interaction}:"+ava_labelnames[avalabel[0]+1]
                else:
                    id_to_ava_labels[tid] = "{Body}:"+ava_labelnames[avalabel[0]+1]

                if avalabel[1] == 60 or avalabel[1] == 79:
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Head}:"+ava_labelnames[avalabel[1]+1]
                elif avalabel[1] in np.arange(63, 79, 1):
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Interaction}:"+ava_labelnames[avalabel[1]+1]
                else:
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Body}:"+ava_labelnames[avalabel[1]+1]

                if avalabel[2] == 60 or avalabel[2] == 79:
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Head}:"+ava_labelnames[avalabel[2]+1]
                elif avalabel[2] in np.arange(63, 79, 1):
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Interaction}:"+ava_labelnames[avalabel[2]+1]
                else:
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Body}:"+ava_labelnames[avalabel[2]+1]

                if avalabel[3] == 60 or avalabel[3] == 79:
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Head}:" + ava_labelnames[avalabel[3] + 1]
                elif avalabel[3] in np.arange(63, 79, 1):
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Interaction}:" + ava_labelnames[avalabel[3] + 1]
                else:
                    id_to_ava_labels[tid] = id_to_ava_labels[tid] + "\n{Body}:" + ava_labelnames[avalabel[3] + 1]

                if avalabel[0] == 60 or avalabel[0] == 79:
                    id_to_ava_labels_file[tid] = "{{Head}:" + ava_labelnames[avalabel[0] + 1]
                elif avalabel[0] in np.arange(63, 79, 1):
                    id_to_ava_labels_file[tid] = "{{Interaction}:" + ava_labelnames[avalabel[0] + 1]
                else:
                    id_to_ava_labels_file[tid] = "{{Body}:" + ava_labelnames[avalabel[0] + 1]

                if avalabel[1] == 60 or avalabel[1] == 79:
                    id_to_ava_labels_file[tid] = id_to_ava_labels_file[tid] + ",{Head}:" + ava_labelnames[
                        avalabel[1] + 1]
                elif avalabel[1] in np.arange(63, 79, 1):
                    id_to_ava_labels_file[tid] = id_to_ava_labels_file[tid] + ",{Interaction}:" + ava_labelnames[
                        avalabel[1] + 1]
                else:
                    id_to_ava_labels_file[tid] = id_to_ava_labels_file[tid] + ",{Body}:" + ava_labelnames[
                        avalabel[1] + 1]
            ...
            action = {}
            for t in id_to_ava_labels_file.keys():
                peo = peo + 1
                peostr = "T"
                action[peostr] = id_to_ava_labels_file[t]
                result[str(m)] = action
    ...
    actionJson = json.dumps(result)
    with open('./result4.txt', 'a') as f:
        f.write(actionJson)
    f.close()
    resultallJson = json.dumps(resultall)
    with open("result3.txt", 'a') as f:
        f.write(resultallJson + '\n')
    f.close()
    outputvideo.release()
...
if __name__=="__main__":
    ...
    parser.add_argument('--input', type=str, default="back-00.00.00.000-00.00.05.150-00.00.00.000-00.00.01.298.mp4", help='test imgs folder or video or camera')
    parser.add_argument('--output', type=str, default="output3.mp4", help='folder to save result imgs, can not use input folder')
